...
FeatureComponent.prototype.constants = {
    ...
    newItem: [{
        ...
        "InstDescription": "Add a short description for instructor that briefly describes the feature.",
        "StudDescription": "Add a short description for students that briefly describes the feature.",
        "resourceUrl": "",
...
FeatureComponent.prototype._validateItem = function(node){
    ...
    newFeature.InstDescription =       node.getElementsByClassName('o-feature-description-inst')[0].textContent.replace(/</g, "&lt;").replace(/>/g, "&gt;");
    newFeature.StudDescription =       node.getElementsByClassName('o-feature-description-stud')[0].textContent.replace(/</g, "&lt;").replace(/>/g, "&gt;");
    ...
    if (newFeature.InstDescription.trim().length == 0 && newFeature.StudDescription.trim().length == 0) {
        alert("Student or Instrocutor Description is Mandatory");
        return null;
    }
...
FeatureComponent.prototype.cancelItem = function (item,event) {
    ...
    for(var i = 0; i < window.$featureData.contents.length ; i++) {
        if(window.$featureData.contents[i].contentId ===item){
            ...
            node.getElementsByClassName('o-feature-description-inst')[0].textContent = window.$featureData.contents[i].InstDescription;
            node.getElementsByClassName('o-feature-description-stud')[0].textContent = window.$featureData.contents[i].StudDescription;
